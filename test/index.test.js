...
test('Attaching creatives and events', function(t){
  ...
  t.equal(creative.trackingEvents[0].type, 'creativeView', 'It should define tracking event types');
...
test('Match elements in the response', function(t){
  response = vast.xml({ pretty : true, indent : '  ', newline : '\n' });
  t.ok(/VAST/.test(response), 'It should have VAST in the response');
  t.ok(/\<Ad id="/.test(response), 'It should have <Ad ...  in the response');
  t.ok(/\<AdSystem/.test(response), 'It should have <AdSystem ... in the response');
  t.ok(/\<AdTitle/.test(response), 'It should have <AdTitle ... in the response');
  t.ok(/\<Impression/.test(response), 'It should have <Impression ... in the response');
  t.ok(/\<Creative/.test(response), 'It should have <Creative ... in the response');
  t.ok(/\<MediaFile/.test(response), 'It should have <MediaFile ... in the response');
  t.end();
});